// server.js — RefundX Proxy (ESM, Node 18+)
// يعمل محليًا بالـ listen، وعلى Vercel كسيرفرلس (هنصدر app فقط)
...
app.get('/proxy/order', verifyProxy, async (req, res)=>{
  try{
    ...
    const data = await adminRest(req.shop, '/orders.json', {
      name:`#${number}`, email, status:'any', fields:'id,currency,line_items'
    });
    const order = data?.orders?.[0];
    if(!order) return res.status(404).json({ error:'not_found' });

    const items = await Promise.all((order.line_items||[]).map(async li=>{
      let handle = '';
      if(li.product_id){
        try { const p = await adminRest(req.shop, `/products/${li.product_id}.json`); handle = p?.product?.handle || ''; } catch{}
      }
      return {
        title: li.name,
        ...
        handle, quantity: li.quantity, sku: li.sku || '', available: true
      };
    }));
...
app.get('/proxy/stock', verifyProxy, async (req, res)=>{
  try{
    ...
    const q = `query($id:ID!){ product(id:$id){ variants(first:100){ nodes{ id title availableForSale } } } }`;
    const data = await adminGraphQL(req.shop, q, { id: productId });
    const variants = data?.product?.variants?.nodes?.map(v=>({ id:v.id, title:v.title, available:v.availableForSale })) || [];
    res.set('Cache-Control','no-store');
    res.json({ variants });
  }catch(e){ res.status(500).json({ error:'server_error', detail:String(e.message||e) }); }
});
...
app.post('/proxy/upload/start', verifyProxy, async (req, res)=>{
  try{
    ...
    const formData = {}; for(const p of (t.parameters||[])) formData[p.name]=p.value;
...
app.post('/proxy/upload/complete', verifyProxy, async (req, res)=>{
  try{
    ...
    const q = `mutation($files:[FileCreateInput!]!){
      fileCreate(files:$files){ files{ url alt } userErrors{ field message } }}`;
